class Prescription < ActiveRecord::Base
  ...
  has_many :prior_authorizations, dependent: :destroy, inverse_of: :prescription
  ...
  def initiate_pa(current_user)
    prior_authorization = prior_authorizations.build(user: current_user,
                                   state: patient.state,
                                   urgent: false)
    response = CoverMyMeds
               .default_client
               .create_request RequestConfigurator.new(prior_authorization).request

    prior_authorization.set_cmm_values(response).save
